...
pub fn collect_hardware_inventory(verbose: bool) -> Option<HardwareInventory> {
    ...
    let device_path_to_text = match uefi::boot::get_handle_for_protocol::<DevicePathToText>() {
        ...
        Err(_) => {
            if verbose {
                write_line("  Warning: DevicePathToText protocol not available, showing raw pointers only");
            }
            None
        }
    };
    ...
    for (index, handle) in handles.iter().enumerate() {
        ...
        if let Ok(device_path_protocol) = uefi::boot::open_protocol_exclusive::<DevicePath>(*handle) {
            ...
            if let Some(ref to_text) = device_path_to_text {
                match to_text.convert_device_path_to_text(&device_path_protocol, DisplayOnly(false), AllowShortcuts(true)) {
                    Ok(text) => {
                        if verbose {
                            ...
                            if let Ok(_) = text.as_str_in_buf(&mut path_buf) {
                                ...
                                write_line(&format!("  Device {}: [Failed to convert to string]", index + 1));
                            }
                        }
                    ...
                    Err(_) => {
                        if verbose {
                            write_line(&format!("  Device {}: Handle 0x{:016x} (failed to convert to text)", 
                ...
                if verbose {
                    write_line(&format!("  Device {}: Handle 0x{:016x} (no text conversion)", 
                        index + 1, core::ptr::addr_of!(*handle) as usize as u64));
                }
            }
        }
        ...
        if devices.len() >= hardware::MAX_HARDWARE_DEVICES {
            if verbose {
                write_line(&format!("  ... (limiting to first {} devices)", hardware::MAX_HARDWARE_DEVICES));
            }
            break;
        }
    }
    ...
    if verbose {
        write_line(&format!("✓ Hardware inventory collected: {} devices", devices.len()));
        write_line(&format!("  Device struct size: {} bytes", core::mem::size_of::<HardwareDevice>()));
        write_line(&format!("  Total inventory size: {} bytes ({:.2} KB)", inventory.total_size, inventory.total_size as f64 / 1024.0));
        write_line(&format!("  Devices array pointer: 0x{:016x}", inventory.devices_ptr));
    } else {
        write_line(&format!("✓ Hardware inventory collected: {} devices", devices.len()));
    }
...
pub fn display_hardware_inventory(inventory: &HardwareInventory) {
    ...
    if inventory.device_count > hardware::MAX_HARDWARE_DEVICES as u32 {
        write_line(&format!("│ Warning: Device count ({}) exceeds maximum ({}) │", 
            inventory.device_count, hardware::MAX_HARDWARE_DEVICES));
    }
    
    write_line(&alloc::format!("│ Total Devices Found: {}                              │", inventory.device_count));
    write_line(&alloc::format!("│ Inventory Size: {} bytes ({:.2} KB)              │", 
        inventory.total_size, inventory.total_size as f64 / 1024.0));
    write_line(&alloc::format!("│ Devices Pointer: 0x{:016x}                    │", inventory.devices_ptr));
    write_line("├─────────────────────────────────────────────────────────┤");
