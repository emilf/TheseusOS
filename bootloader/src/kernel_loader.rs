...
pub fn find_kernel_file() -> Result<RegularFile, Status> {
    ...
    let mut fs = get_image_file_system(image_handle).map_err(|e| {
        write_line(&format!("✗ Failed to get file system: {:?}", e));
        e.status()
    })?;

    // Open the root directory
    let mut root_dir = fs.open_volume().map_err(|e| {
    ...
    let file_handle = root_dir
        .open(kernel_path, FileMode::Read, FileAttribute::READ_ONLY)
        .map_err(|e| {
            write_line(&format!("✗ Failed to open kernel file: {:?}", e));
            e.status()
        })?;

    let kernel_file = file_handle.into_regular_file().ok_or_else(|| {
...
pub fn get_kernel_file_info(file: &mut RegularFile) -> Result<u64, Status> {
    ...
    let file_info = file.get_info::<FileInfo>(&mut info_buffer).map_err(|e| {
    ...
    write_line(&format!(
        "Kernel file size: {} bytes ({:.2} MB)",
        file_size,
        file_size as f64 / theseus_shared::constants::memory::BYTES_PER_MB
    ));

    if file_size > kernel::MAX_KERNEL_SIZE {
        write_line(&format!(
            "✗ Kernel file too large: {} bytes (max: {} bytes)",
            file_size,
...
pub fn read_kernel_binary(file: &mut RegularFile, file_size: u64) -> Result<Vec<u8>, Status> {
    ...
    let bytes_read = file.read(&mut kernel_data).map_err(|e| {
    ...
    if bytes_read != file_size as usize {
        write_line(&format!(
            "✗ Incomplete read: expected {} bytes, got {} bytes",
...
pub fn analyze_kernel_binary(kernel_data: &[u8]) -> Result<KernelInfo, Status> {
    ...
    for i in 0..ph_num {
        ...
        if segment_type == 1 {
            ...
            let flags = u32::from_le_bytes([
                kernel_data[ph_start + 4],
                kernel_data[ph_start + 5],
                kernel_data[ph_start + 6],
                kernel_data[ph_start + 7],
            ]);
            ...
            if virt_addr < min_virtual_address {
                min_virtual_address = virt_addr;
            }
            let end = virt_addr.saturating_add(mem_size);
            if end > max_virtual_end {
                max_virtual_end = end;
            }

            write_line(&format!(
                "    {}: 0x{:016X} ({} bytes, flags: 0x{:X})",
    ...
    if min_virtual_address == u64::MAX {
        return Err(Status::INVALID_PARAMETER);
    }
    ...
    write_line(&format!(
        "  Virtual span size: {} bytes ({:.2} MB)",
        total_span_size,
        total_span_size as f64 / theseus_shared::constants::memory::BYTES_PER_MB
    ));
...
pub fn load_kernel_sections(
    ...
    write_line(&format!(
        "Loading kernel at physical address: 0x{:016X}",
        physical_base
    ));
    ...
    for section in &mut kernel_info.sections {
        ...
        section.physical_address =
            (section.virtual_address as i64 + virtual_to_physical_offset) as u64;
        ...
        if section.name == ".bss" {
            ...
            write_line(&format!(
                "    BSS section: {} bytes (zeroing)",
                section.size
            ));
            write_line(&format!(
                "    BSS physical address: 0x{:016X}",
                section.physical_address
            ));
            ...
            if section.file_offset > 0 || section.name != ".bss" {
                ...
                if file_size > 0 {
                    let file_data = &kernel_data
                        [section.file_offset as usize..(section.file_offset + file_size) as usize];
...
fn apply_relocations(
    ...
    for i in 0..e_shnum {
        ...
        if sh_offset == 0 || sh_size == 0 || sh_entsz == 0 {
            continue;
        }
        if sh_offset + sh_size > kernel_data.len() {
            write_line("✗ RELA section out of bounds");
            return Err(Status::INVALID_PARAMETER);
        }

        // Locate linked symbol table
        if sh_link >= e_shnum {
            write_line("✗ RELA link index out of range");
            return Err(Status::INVALID_PARAMETER);
        }
        let symtab_base = sh_at(sh_link);
        let symtab_type = read_u32(kernel_data, symtab_base + 4);
        // SYMTAB=2 or DYNSYM=11
        if symtab_type != 2 && symtab_type != 11 {
            write_line("✗ Linked symtab has unexpected type");
            return Err(Status::INVALID_PARAMETER);
        }
        ...
        if sym_off == 0
            || sym_size == 0
            || sym_entsize == 0
            || sym_off + sym_size > kernel_data.len()
        {
        ...
        while off + sh_entsz <= sh_offset + sh_size {
            ...
            if target_phys == 0 {
                write_line("✗ Relocation target phys is 0");
                return Err(Status::INVALID_PARAMETER);
            }
            ...
            let value: u64 = match r_type {
                1 => {
                    // R_X86_64_64: S + A
                    if r_sym == 0 || sym_entsize == 0 {
                        write_line("✗ R_X86_64_64 with no symbol");
                        return Err(Status::INVALID_PARAMETER);
                    }
                    let sym_entry_off = sym_off + r_sym * sym_entsize;
                    if sym_entry_off + sym_entsize > sym_off + sym_size {
                        write_line("✗ Symbol index out of range");
                        return Err(Status::INVALID_PARAMETER);
                    }
                ...
                7 => {
                    // R_X86_64_RELATIVE: B + A (B = image virtual base)
            ...
            unsafe {
                (target_phys as *mut u64).write_volatile(value);
            }
            applied_here += 1;
            off += sh_entsz;
        }
    ...
    let mut dyn_rel_entries = 0usize;
    {
        ...
        if kernel_data.len() >= 0x40 + 8 {
            ...
            if e_phoff != 0
                && e_phentsize >= 56
                && e_phoff + e_phnum * e_phentsize <= kernel_data.len()
            {
                ...
                if dyn_off != 0
                    && dyn_size >= 16
                    && dyn_off + dyn_size as usize <= kernel_data.len()
                {
                    // Parse dynamic tags
                    ...
                    while cursor + 16 <= dyn_off + dyn_size as usize {
                        let d_tag = i64::from_le_bytes([
                            kernel_data[cursor],
                            kernel_data[cursor + 1],
                            kernel_data[cursor + 2],
                            kernel_data[cursor + 3],
                            kernel_data[cursor + 4],
                            kernel_data[cursor + 5],
                            kernel_data[cursor + 6],
                            kernel_data[cursor + 7],
                        ]);
                        let d_val = u64::from_le_bytes([
                            kernel_data[cursor + 8],
                            kernel_data[cursor + 9],
                            kernel_data[cursor + 10],
                            kernel_data[cursor + 11],
                            kernel_data[cursor + 12],
                            kernel_data[cursor + 13],
                            kernel_data[cursor + 14],
                            kernel_data[cursor + 15],
                        ]);
                        if d_tag == 0 {
                            break;
                        } // DT_NULL
                    ...
                    let v2p = |va: u64| -> u64 {
                        (va as i64 + (physical_base as i64 - kernel_info.virtual_base as i64))
                            as u64
                    };

                    // Access .dynsym table if present
                    let (symtab_ptr, _symtab_size, symtab_entsz) =
                        if dt_symtab != 0 && dt_syment != 0 {
                    ...
                    if dt_rela != 0 && dt_relasz != 0 {
                        ...
                        for _ in 0..rela_cnt {
                            ...
                            let r_info =
                                unsafe { core::ptr::read_unaligned((rptr + 8) as *const u64) };
                            let r_addend =
                                unsafe { core::ptr::read_unaligned((rptr + 16) as *const i64) };
                            let r_type = (r_info & 0xFFFFFFFF) as u32;
                            let r_sym = (r_info >> 32) as u32 as usize;
                            ...
                            let value: Option<u64> = match r_type {
                                1 => {
                                    ...
                                    if symtab_ptr != 0 && symtab_entsz >= 24 && r_sym != 0 {
                                        ...
                                        // st_value at +8
                                        let st_value = unsafe {
                                            core::ptr::read_unaligned((sym_off + 8) as *const u64)
                                        };
                            ...
                            if let Some(val) = value {
                                unsafe {
                                    (target_phys as *mut u64).write_volatile(val);
                                }
                                dyn_rela_entries += 1;
                            }
    ...
    write_line(&format!(
        "✓ Relocations applied: sections={}, entries={}, dyn_rela={}, dyn_rel={}",
        rela_sections, rela_entries, dyn_rela_entries, dyn_rel_entries
    ));
    Ok(())
}
...
pub fn load_kernel_binary(
...
) -> Result<(u64, u64, u64, u64), uefi::Status> {
    ...
    let memory_region =
        crate::memory::allocate_memory(alloc_size, uefi::mem::memory_map::MemoryType::LOADER_DATA)?;
    ...
    write_line(&format!(
        "  Entry point (physical): 0x{:016X}",
        physical_entry
    ));
    write_line(&format!(
        "  Entry point (virtual): 0x{:016X}",
