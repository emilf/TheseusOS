        ...
        if handoff_addr != 0 {
            ...
            if handoff.size > 0 {
                ...
                match theseus_kernel::validate_handoff(handoff) {
                    Ok(()) => {
                        theseus_kernel::display::kernel_write_line("Handoff validation passed")
                    }
                    Err(msg) => {
                ...
                // - CPU features
                // - All kernel services
                theseus_kernel::setup_kernel_environment(
                    handoff,
                ...
                theseus_kernel::display::kernel_write_line(
                    "All kernel tests completed successfully",
                );
            } else {
                theseus_kernel::display::kernel_write_line(
                    "ERROR: Handoff structure has invalid size",
                );
            }
        } else {
