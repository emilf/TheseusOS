...
impl AcpiHandler for KernelAcpiHandler {
    ...
    unsafe fn map_physical_region<T>(
        ...
        let virtual_address = (PHYS_OFFSET + physical_address as u64) as *mut T;
...
pub fn initialize_acpi(acpi_rsdp: u64) -> Result<PlatformInfo, &'static str> {
    kernel_write_line("Initializing ACPI...");
    ...
    kernel_write_line("  RSDP address: 0x");
    ...
    kernel_write_line("  Parsing ACPI tables...");
    let tables = unsafe { AcpiTables::from_rsdp(handler, acpi_rsdp as usize) }
        .map_err(|_| "Failed to parse ACPI tables")?;
    
    kernel_write_line("  ACPI tables parsed successfully");
...
fn extract_platform_info(tables: &AcpiTables<KernelAcpiHandler>) -> Result<PlatformInfo, &'static str> {
    kernel_write_line("  Extracting platform information...");
    
    let mut platform_info = PlatformInfo::new();
    
    // Get platform info from acpi crate
    let platform_info_acpi = tables.platform_info()
...
fn parse_rsdp(rsdp_address: u64) -> Result<u64, &'static str> {
    ...
    unsafe {
        ...
        if &sig_bytes != b"RSD PTR " {
            kernel_write_line(&alloc::format!("  Invalid RSDP signature: {:02X?}", sig_bytes));
            return Err("Invalid RSDP signature");
        }
        ...
        if xsdt_addr_2_0 != 0 {
            kernel_write_line(&alloc::format!("  Using XSDT (ACPI 2.0): 0x{:016X}", xsdt_addr_2_0));
            Ok(xsdt_addr_2_0)
        } else if rsdt_addr_1_0 != 0 {
            kernel_write_line(&alloc::format!("  Using RSDT (ACPI 1.0): 0x{:016X}", rsdt_addr_1_0));
...
fn find_madt(rsdt_address: u64) -> Result<u64, &'static str> {
    ...
    unsafe {
        ...
        kernel_write_line(&alloc::format!("  {} entries: {}", if is_xsdt { "XSDT" } else { "RSDT" }, entry_count));
...
fn parse_madt(madt_address: u64) -> Result<PlatformInfo, &'static str> {
    ...
    unsafe {
        ...
        kernel_write_line(&alloc::format!("  Local APIC address: 0x{:016X}", local_apic_address));
        ...
        while offset < madt_length as usize {
            ...
            match entry_type {
                ...
                1 => { // IO APIC
                    ...
                    let io_apic_address = core::ptr::read_volatile((madt_ptr.add(offset + 4)) as *const u32) as u64;
                    let gsi_base = core::ptr::read_volatile((madt_ptr.add(offset + 8)) as *const u32);
                    ...
                    kernel_write_line(&alloc::format!("    IO APIC ID: {}, Address: 0x{:016X}, GSI Base: {}", 
                        io_apic_id, io_apic_address, gsi_base));
                }
                _ => {
        ...
        kernel_write_line(&alloc::format!("  IO APICs: {}", platform_info.io_apic_count));
