...
- With the corrected route string and TRB encodings the controller now posts transfer events for both descriptor TDs, `evaluate-context` returns completion code 1, `configure-endpoint` succeeds, and the logged device descriptor reflects QEMU’s virtual keyboard (`usb=0x0200 vid=0x0627 pid=0x0001`).

## Next Steps
1. Extend the control-path scaffolding to fetch and parse the configuration/interface/HID descriptors, select the active configuration, and seed the interrupt endpoint contexts for the keyboard.
