...
- **EP0 event handling**: Command-completion polling now delegates to a shared event-ring reader that recognises transfer events, caches EP0 completions, and follows the spec path: issue `ADDRESS_DEVICE` with `BSR=1`, request the first 8 bytes of the device descriptor, update EP0’s Max Packet Size via `EVALUATE_CONTEXT`, issue `ADDRESS_DEVICE` with `BSR=0`, and only then fetch the full descriptor. Even with this flow the controller never posts a transfer event, so the driver logs the descriptor TRBs, event-ring snapshot, and post-failure port states to aid further debugging.
...
- The EP0 descriptor requests still trigger an event-ring cycle mismatch (cycle bit remains 0 while `USBSTS.TRANSFER_EVENT` is asserted). Even after the Address/Evaluate sequence the event ring only shows port-status and command-completion entries; the EP0 TRBs remain untouched. The latest logs now capture the full eight-entry event-ring dump, the descriptor TD contents, and the post-failure port states to pinpoint what the controller saw.

## Next Steps
1. Investigate the EP0 transfer ring mismatch in QEMU (compare the Address/Evaluate TD sequence against a known-good stack, verify the dequeue pointer/DCS handshake, and confirm the controller transitions EP0 to the Running state) so the device descriptor transfer produces a valid event.
