...
- **Ring staging**: The nascent xHCI driver allocates DMA-backed command/event rings, programs CRCR/ERST for interrupter 0, and logs their physical addresses so later milestones can plug in TRB programming quickly (`kernel/src/drivers/usb/xhci/mod.rs:300`).
