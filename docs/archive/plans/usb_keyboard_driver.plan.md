...
- Teach the shared event ring to separate command completions from EP0 transfer events, cache control-transfer completions, and follow the spec-mandated enumeration path: `ADDRESS_DEVICE` with `BSR=1`, partial 8-byte `GET_DESCRIPTOR`, `EVALUATE_CONTEXT` to apply the true Max Packet size, a second `ADDRESS_DEVICE` with `BSR=0`, and finally the full descriptor. (Despite the richer diagnostics—event-ring snapshots, descriptor TRBs, and port summaries—the controller still never raises the transfer event, so further work must compare our TD layout against a known-good implementation.)
- Ensure the slot context advertises both the slot and EP0 endpoint in the Context Entries field and seed the endpoint context with the correct TR dequeue pointer/cycle state so the controller has a valid view before the first doorbell. (Despite the correction, QEMU still fails to emit the transfer event; additional controller-side investigation is required.)
