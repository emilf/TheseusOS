...
- ☐ Create an xHCI driver module that maps the controller MMIO region, parses capability/operational registers, and performs controller reset to a known state.
  - Stub driver now maps MMIO, performs the halt/reset handshake, reports capabilities, allocates command/event rings, and programs CRCR/ERST for interrupter 0 (`kernel/src/drivers/usb/xhci/mod.rs:1`); runtime bring-up remains.
