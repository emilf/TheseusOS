...
- Ensure the slot context advertises both the slot and EP0 endpoint in the Context Entries field and seed the endpoint context with the correct TR dequeue pointer/cycle state so the controller has a valid view before the first doorbell. (The HID boot keyboard’s interrupt-IN endpoint context now sits alongside EP0 with its transfer ring allocated, input context staged, and the slot doorbell armed; next up is harvesting transfer events and parsing the HID reports.)
