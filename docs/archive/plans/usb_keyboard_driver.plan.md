...
- Teach the shared event ring to separate command completions from EP0 transfer events, cache control-transfer completions, and synchronously request the device descriptor right after `ADDRESS_DEVICE`. (Current runs still show no transfer event—even after chaining the data stage, setting `TD Size`, and issuing a follow-up `Configure Endpoint` command—so the driver now captures full event-ring dumps, the descriptor TD contents, and port states to inform the next debugging pass.)
