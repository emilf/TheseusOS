...
Once `kernel_entry` is called, control never returns to firmware. The kernel stabilizes the platform, maps its address space, brings up interrupts, and hands off to higher-level services. This page summarizes the major phases and where to look in the source.
...
1. **Interrupt lockdown** — `interrupts::disable_all_interrupts()` disables/masks the early interrupt paths used during bring-up, but this page should not be read as a magical guarantee that absolutely every architectural interrupt source (especially NMI-class behavior) is gone forever just because one helper ran.
...
3. **Control registers** — `cpu::setup_control_registers()` performs the limited CR0/CR4 setup used by the current bring-up path before paging changes.
...
- If the jump path fails unexpectedly, the kernel aborts via `boot::abort_with_context`.
...
6. **UEFI runtime** — the current bring-up path includes runtime-services virtual-address-map work and a firmware-time query, but this should not be read as proof that UEFI runtime is already a fully settled long-term subsystem design.
...
- Depending on `config`, the kernel:
  ...
  - may run the COM1 reverse-echo debug loop (`serial_debug::run_reverse_echo_session()`),
  - and either idles with `hlt` in a loop (`config::KERNEL_SHOULD_IDLE`) or exits QEMU via the `isa-debug-exit` device.
